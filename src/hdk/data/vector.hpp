#ifndef HDK_VECTOR_HPP
#define HDK_VECTOR_HPP
...
namespace hdk
{
	...
	public:
		 ...
		 *
		 * Example:
		 * hdk::vec3{1.0f, 2.0f, 3.0f}.swizzle<0, 1, 0, 1>() == hdk::vec4{1.0f, 2.0f, 1.0f, 2.0f}
		 ...
		 *
		 * Example:
		 * hdk::vec2{1.0f, 2.0f}.with_more(3.0f) == hdk::vec3{1.0f, 2.0f, 3.0f}
		 ...
		 *
		 * Example:
		 * hdk::vec2{0.0f, 1.0f}.with_more(hdk::vec3{2.0f, 3.0f, 4.0f}) == hdk::vector<float, 5>{0.0f, 1.0f, 2.0f, 3.0f, 4.0f}
		 ...
		 */
		template<hdk::numeric X, typename = std::enable_if_t<!std::is_same_v<T, X>>>
...
{
	template<hdk::numeric T, std::size_t S>
	struct hash<hdk::vector<T, S>>
	{
		std::size_t operator()(const hdk::vector<T, S>& v) const
		{
			std::size_t ret = 0;
